...
import { ApiResponse, CreateWritingSubPart, WritingSubPart } from '@/utils/types/types'
...
export const useCreateWritingSubPart = () => {
	...
	return useMutation({
		mutationFn: async (data: CreateWritingSubPart) =>
			(await axiosPrivate.post<ApiResponse<WritingSubPart>>(writingSubPartEndpoints.all, data)).data,
		onSuccess: () => {
