...
import { ApiResponse, CreateSpeakingSubPart, SpeakingSubPart } from '@/utils/types/types'
...
export const useCreateSpeakingSubPart = () => {
	...
	return useMutation({
		mutationFn: async (data: CreateSpeakingSubPart) =>
			(await axiosPrivate.post<ApiResponse<SpeakingSubPart>>(speakingSubPartEndpoints.all, data)).data,
		onSuccess: () => {
