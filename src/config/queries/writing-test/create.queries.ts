...
import { ApiResponse, CreateWritingTest, WritingTest } from '@/utils/types/types'
...
export const useCreateWritingTest = () => {
	...
	return useMutation({
		mutationFn: async (data: CreateWritingTest) =>
			(await axiosPrivate.post<ApiResponse<WritingTest>>(writingTestEndpoints.all, data)).data,
		onSuccess: () => {
