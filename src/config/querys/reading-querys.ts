  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<Position[]>(
        `${readingEndpoints.all}?limit=99&page=${page}`
      );
      return data;
    },
  });
...
export const useUpdatePosition = () => {
  ...
  return useMutation({
    mutationFn: async ({
      id,
      ...data
    }: Partial<CreatePosition> & { id: string }) =>
      (await api.patch<ApiResponse<Position>>(readingEndpoints.one(id), data))
        .data,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [readingEndpoints.all] });
      notification.success({
...
export const useDeletePosition = () => {
  ...
  return useMutation({
    mutationFn: async (id: string) =>
      (await api.delete(readingEndpoints.one(id))).data,
    onSuccess() {
