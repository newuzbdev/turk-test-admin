...
export default function ListeningEditor() {
  ...
  const { mutateAsync: createListeningTest, isPending: isCreating } =
    useCreateListeningTestWithAddition();
  const { data: listeningTest, isLoading } = useGetOneListeningTest(
    isNewTest ? "" : id || ""
  );
  ...
  useEffect(() => {
    if (isNewTest && initialTestData) {
      setTestData((prev) => ({
        ...
        description: `${initialTestData.title} - Listening Test`,
      }));
    }
  }, [isNewTest, initialTestData]);
  ...
  const transformDataForAPI = (data: ListeningTestData) => {
    return {
      title: data.title,
      ...
      parts: data.parts.map((part) => ({
        ...
        sections: (part.sections || []).map((section, sectionIndex) => ({
          ...
          imageUrl: "", // Default empty string since Section type doesn't have imageUrl
          questions: (section.questions || []).map(
            (question, questionIndex) => ({
              number: questionIndex + 1, // Use index + 1 for question number
              type: question.type || "MULTIPLE_CHOICE",
              text: question.question || `Question ${questionIndex + 1}`,
              answers: (question.answers || []).map((answer, answerIndex) => {
                // Generate variant text (A, B, C, D, etc.)
                const variantText = String.fromCharCode(65 + answerIndex); // A=65, B=66, etc.
                return {
                  variantText: variantText,
                  answer: answer.answer || `Option ${variantText}`,
                  correct: Boolean(answer.isCorrect), // Use correct property name
                };
              }),
            })
          ),
  ...
  const handleSave = async () => {
    try {
      ...
      await createListeningTest(transformedData);
      navigate("/listening");
    } catch (error) {
  ...
  if (!isNewTest && !listeningTest?.data) {
    return (
      <div className="flex items-center justify-center h-screen">
  ...
  const displayTitle = isNewTest ? testData.title : listeningTest?.data?.title;
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2">
            <h1 className="text-lg font-semibold">
              {displayTitle || "New Listening Test"}
            </h1>
