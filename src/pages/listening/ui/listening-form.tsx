...
import type { CreateTest, Part } from "@/utils/types/types";
...
export default function ListeningForm({
  ...
  const { data: ieltsList, isLoading } = useGetAllIelts();
  ...
  const updatePart = (index: number, updated: Part) => {
  ...
  const addPart = () => {
    const newPart: Part = {
      title: "",
      testId: "",
      order: (formData.parts?.length || 0) + 1,
      sections: [],
    };
